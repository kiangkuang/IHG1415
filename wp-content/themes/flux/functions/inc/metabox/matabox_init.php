			...
			switch ( $field['type'] ) {
				...
				case 'images':
					...
					foreach ($field['options'] as $key => $option) {
						echo '<div class="cmb_radio_inline_option_images">';
						$i++;
						
						$checked = '';
						$selected = '';
						if(NULL!=checked($meta, $key, false)) {
							$checked = checked($meta, $key, false);
							$selected = 'radio-img-selected';  
						}
						
						echo'<input type="radio" id="radio-img-' . $field['id'] . $i . '" class="checkbox radio-img-radio" value="'.$key.'" name="'.$field['id'].'" '.$checked.'/>
						<img src="'.$option.'" class="radio-img-img '. $selected .'" onClick="document.getElementById(\'radio-img-'. $field['id'] . $i.'\').checked = true;"  title="'.$key.'" alt=""  />
						</div>';
				...
				case 'select_sidebar':
					...
					foreach ( $wp_registered_sidebars as $sidebar ) {
						if( empty( $meta ) && !empty( $field['std'] ) ) $meta = $field['std'];
						echo '<option value="', $sidebar['id'], '"', $meta == $sidebar['id'] ? ' selected="selected"' : '', '>', $sidebar['name'], '</option>';
					}
					echo '</select>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
