...
class Options_Machine {
	...
	public static function optionsframework_add_item_function($id,$std){

	    $smof_data = of_get_options();
	    $smof_data = of_get_options();
		
		$uploader = '';
	    $additem = $smof_data[$id];
		
		
	    if ( $additem != "") { $val = $additem; } else {$val = $std;}
	    
		$uploader .= '<input type="text" name="'. $id .'" id="'. $id .'_additem" value="'. $val .'" />';	
		
		$uploader .= '<div class="upload_button_divxxx"><span class="button media_upload_buttonxx" id="'.$id.'" rel="' . $int . '">Add</span>';
